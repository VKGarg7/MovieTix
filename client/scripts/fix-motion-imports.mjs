...
function processFile(filePath) {
  ...
  if (!framerImportMatch) {
    skipped.push(`${filePath}: no framer-motion import found`);
    return;
  }
  ...
  // Check for typo like "Alphabeticallymotion"
  const typoMatch = importedNames.find((n) => n.toLowerCase().includes("motion") && n !== "motion");
  if (typoMatch) {
    const newContent = content.replace(
      new RegExp(`\\b${typoMatch}\\b`),
      "motion"
    );
    writeFileSync(filePath, newContent);
    fixed.push(`${filePath}: fixed typo "${typoMatch}" -> "motion"`);
    return;
  }

  // Add motion to the import
  const newContent = content.replace(
    /import\s*\{([^}]*)\}\s*from\s*["']framer-motion["']/,
    (match, names) => {
      const trimmed = names.trim();
      return `import { motion, ${trimmed} } from "framer-motion"`;
    }
  );

  writeFileSync(filePath, newContent);
  fixed.push(`${filePath}: added motion to import`);
}
...
if (skipped.length > 0) {
  console.log("\n=== SKIPPED (no framer-motion import) ===");
